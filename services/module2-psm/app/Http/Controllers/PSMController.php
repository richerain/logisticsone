...
class PSMController extends Controller
{
    ...
    public function updateQuote(Request $request, $id)
    {
        try {
            ...
            // Synchronize status with purchase request
            $this->syncPurchaseStatusFromQuote($quote);
    ...
    // Synchronize purchase status from quote status
    private function syncPurchaseStatusFromQuote(Quote $quote)
    {
        try {
            $purchase = Purchase::where('request_id', $quote->request_id)->first();
            
            if ($purchase) {
                // Map quote status to purchase status
                $statusMapping = [
                    'pending' => 'Pending',
                ...
                $purchaseStatus = $statusMapping[$quote->status] ?? 'Pending';
                ...
                Log::info("Purchase status synchronized from quote", [
            ...
            Log::error("Failed to synchronize purchase status from quote: " . $e->getMessage());
        }
    }
    ...
    public function getPurchaseRequestsForQuotes(Request $request)
    {
        try {
            $purchases = Purchase::whereNotIn('status', ['Rejected'])
                ...
                ->get(['purchase_id', 'request_id', 'vendor', 'item', 'quantity', 'units', 'unit_price', 'total_quote', 'expected_delivery', 'description', 'quote_date', 'status', 'branch', 'po_number']);
    ...
    public function createPurchaseRequest(Request $request)
    {
        $request->validate([
            ...
            // quote_date and status removed from validation - will be auto-set
        ]);

        try {
            // Auto-calculate total quote based on quantity, units, and unit price
            $totalQuote = $request->quantity * $request->units * $request->unit_price;
            
            $purchaseData = array_merge($request->all(), [
                'total_quote' => $totalQuote,
                'quote_date' => now()->toDateString(), // Auto-set current date
    ...
    public function updatePurchaseRequest(Request $request, $id)
    {
        try {
            ...
            $request->validate([
                ...
                'description' => 'nullable|string'
                // Remove status from update - it can only be updated via quotes
            ]);

            // Auto-calculate total quote when quantity, units, or unit_price changes
            $updateData = $request->except(['quote_date', 'status']);
            
            if ($request->hasAny(['quantity', 'units', 'unit_price'])) {
                $quantity = $request->quantity ?? $purchase->quantity;
                $units = $request->units ?? $purchase->units;
                $unitPrice = $request->unit_price ?? $purchase->unit_price;
                $updateData['total_quote'] = $quantity * $units * $unitPrice;
            }
